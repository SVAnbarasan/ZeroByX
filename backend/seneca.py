...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('seneca.log')
    ]
)
...
def get_model(agent_type='seneca'):
    """
...
async def get_seneca_response_async(prompt: str, agent_type='seneca') -> str:
    """
    Async version of get_seneca_response
    """
...
Respond with forensic details and IOCs."""
        ...
        logger.error(f"Error in get_seneca_response: {str(e)}")
        raise

def get_seneca_response(prompt: str, agent_type='seneca') -> str:
    """
    Synchronous wrapper for get_seneca_response_async
    """
    try:
        ...
        response = loop.run_until_complete(get_seneca_response_async(prompt, agent_type))
...
if __name__ == "__main__":
    try:
        ...
        # Get agent type from environment variable or default to seneca
        agent_type = os.environ.get('AGENT_TYPE', 'seneca')
            
        # Ensure we have a response
        response = get_seneca_response(user_prompt, agent_type)
        if not response:
