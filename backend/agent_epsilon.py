...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('agent_epsilon.log')
    ]
)
...
def get_model(agent_type='epsilon'):
    """
    ...
    if _model_instance is None:
        ...
        try:
            system_message = """
            ### ROLE ###
You are Agent Epsilon, a specialized malware analysis and reverse engineering expert.

### CONTEXT ###
Your role is to help users understand malware analysis, reverse engineering, and threat intelligence. Focus on:
...
### TASK ###
Provide detailed technical analysis and guidance on malware and reverse engineering topics.

### INSTRUCTIONS ###
Your response should follow this structure:
1. **Analysis Overview**: Brief description of the malware/exploit
2. **Technical Details**: 
   - File Information
   - Behavioral Analysis
   - Code Analysis
...
### OUTPUT FORMAT ###
Structured response with clear sections and technical details.
            """
...
async def get_epsilon_response_async(prompt: str, agent_type='epsilon') -> str:
    """
    Async version of get_epsilon_response
    """
...
Structured response with clear sections and technical details.
        ...
        logger.error(f"Error in get_epsilon_response: {str(e)}")
        raise

def get_epsilon_response(prompt: str, agent_type='epsilon') -> str:
    """
    Synchronous wrapper for get_epsilon_response_async
    """
    try:
        ...
        response = loop.run_until_complete(get_epsilon_response_async(prompt, agent_type))
...
if __name__ == "__main__":
    try:
        ...
        # Get agent type from environment variable or default to epsilon
        agent_type = os.environ.get('AGENT_TYPE', 'epsilon')
            
        # Ensure we have a response
        response = get_epsilon_response(user_prompt, agent_type)
        if not response:
